 ...
 * Chargement dynamique depuis reservations.csv (GitHub) avec fallback local.
 */
  ...
  for (let m = 0; m < 12; m++) {
    ...
    for (let d = 1; d <= lastDay.getDate(); d++) {
      ...
      if (!info) {
        ...
        // Orange pour le check-in
        const bookColor = info.booking.isOwner ? 'var(--owner)' : 'var(--tenant)';
        cls  += ' day-checkin has-booking';
        style = `background: linear-gradient(to right, transparent 0%, transparent 40%, #FF8C00 40%, #FF8C00 100%);`;
        ...
        // Orange pour le check-out
        const bookColor = info.booking.isOwner ? 'var(--owner)' : 'var(--tenant)';
        cls  += ' day-checkout has-booking';
        style = `background: linear-gradient(to right, #FF8C00 0%, #FF8C00 60%, transparent 60%, transparent 100%);`;
        bid   = `data-bid="${BOOKINGS.indexOf(info.booking)}"`;
        inner = `<span class="co-badge" title="Départ">◀</span><span class="day-num" style="color:#fff">${d}</span>`;
      } else if (info.type === 'both') {
        // Rouge si check-in et check-out le même jour (back-to-back)
        cls  += ' day-both has-booking';
        style = `background: #E53935;`;
        bid   = `data-bid="${BOOKINGS.indexOf(info.booking)}"`;
        inner = `<span class="co-badge" title="Départ/Arrivée">◀▶</span><span class="day-num" style="color:#fff">${d}</span>`;
      } else {
        // Milieu de séjour — couleur réservation
